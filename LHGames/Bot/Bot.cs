        ...
        internal void getInterestingObjects(Map map, Point ownPosition)
        {
            ...
            for (int i = 0; i < visibleTiles.Count; i++)
            {
               ...
               case (int)TileContent.House:
                  if (currentTile.Position == PlayerInfo.HouseLocation && PlayerInfo.CarriedResources != 0)
                  {
                     currentObject = new InterestingObject() { distanceFromUser = new int[2] {currentTile.Position.X - PlayerInfo.Position.X, currentTile.Position.Y - PlayerInfo.Position.Y}, position = currentTile.Position };
